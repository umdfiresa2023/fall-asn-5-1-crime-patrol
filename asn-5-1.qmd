...
Your team should have used paneldata.csv instead since it has observations for every county every day. In the data set that you used in this exercise, only observations with crime are recorded.
...
Please see my suggestion below, with a balanced panel data: (-1)

```{r}
ggplot(df, aes(x = TLML, y = total_violent_crime)) + geom_point() + geom_smooth(method="lm", se=FALSE)
```
...
```{r}
fit <- lm(total_violent_crime~TLML, df)
summary(fit)
```
...
Please see my suggestion below: (-1)
